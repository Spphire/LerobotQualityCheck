...
def final_episode_status_from_db(labels_path):
    conn = sqlite3.connect(labels_path)
    conn.row_factory = sqlite3.Row
    rows = conn.execute(
        """
        SELECT episode_index, user, status, updated_at
        FROM labels
...
def final_episode_status_from_json(labels_path):
    ...
    canonical_labels = store.get("labels") or {}
    if isinstance(canonical_labels, dict) and canonical_labels:
        final = {}
        for key, label in canonical_labels.items():
            ...
            if status not in {"accept", "reject"}:
                continue
            try:
                episode_index = int(label.get("episode_index", key))
...
def final_episode_status(labels_path):
    ...
    if labels_path.suffix == ".db":
        return final_episode_status_from_db(labels_path)
